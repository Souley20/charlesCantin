...
export const defOnAcquiredLock = ({ filepath, headerKey, headerValue, sources }) => {
    const sourcesToProcess = new Set(sources);
    const onAcquiredLock = async (release) => {
        debug(`🔒 acquired lock on ${filepath}. Will attach ${headerKey}`);
        const str = fs.readFileSync(filepath, { encoding: 'utf8' });
        const obj_in = JSON.parse(str);
        const obj_out = { ...obj_in, headers: [] };
        if (obj_in.headers) {
 ...
 */
export const updateVercelJSON = async (config) => {
    ...
    if (!headerValue) {
        return {
            error: new Error(`${ERR_PREFIX}: the value of ${headerKey} is an empty string`)
        };
    }
