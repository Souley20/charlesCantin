...
const isBody = (json) => json.tagName === 'body';
...
// for debugging
export const tap = (json) => {
    console.log('=== Himalaya tap ===');
    console.log(json);
    // console.log(JSON.stringify(json, null, 2))
    return json;
};
const propertyValue = (elem, property) => {
...
const eventHandlerContents = (json) => {
    const contents = [];
    const recur = (node) => {
...
const getInlineEventHandlerContents = (json) => {
    return json
        .filter(isHtml)
        .flatMap(children)
        .filter(isBody)
...
const getInlineStyleContents = (json) => {
    return json
        .filter(isHtml)
        .flatMap(children)
        .filter(isBody)
 ...
 */
const himalayaJson = async (filepath) => {
    try {
 ...
 * @internal
 */
export const scriptTagsContents = async (filepath) => {
 ...
 * @internal
 */
export const styleTagsContents = async (filepath) => {
 ...
 * @internal
 */
export const inlineEventHandlerContents = async (filepath) => {
 ...
 * @internal
 */
export const inlineStyleContents = async (filepath) => {
    debug(`Look for inline styles in <body> in ${filepath}`);
    const json = await himalayaJson(filepath);
    return getInlineStyleContents(json);
};
//# sourceMappingURL=html-parsers.js.map