...
import { cspDirectives, validationErrorOrWarnings } from '@jackdbd/content-security-policy';
import { updateHeaders, updateVercelJSON } from '@jackdbd/hosting-utils';
import { DEBUG_PREFIX, DEFAULT_OPTIONS, ERR_PREFIX, SUPPORTED_HOSTING_PROVIDERS, WARN_PREFIX } from './constants.js';
 ...
 */
export const contentSecurityPolicyPlugin = (eleventyConfig, options) => {
    eleventyConfig.on('eleventy.after', async () => {
        debug('plugin options (provided by the user) %O', options);
        const result = schema.default(DEFAULT_OPTIONS).safeParse(options);
        if (!result.success) {
            const allowDeprecatedDirectives = options.allowDeprecatedDirectives ||
        ...
        const { excludePatterns, globPatterns, includePatterns, jsonRecap, reportOnly } = config;
        ...
        let hosting_provider_config_files = [];
        ...
        debug(`will look for inlined JS/CSS in HTML pages matching these patterns %O`, patterns);
        ...
        if (hosting === 'vercel') {
            ...
            throw new Error(`${ERR_PREFIX}: ${hosting} is not a supported hosting provider. This plugin supports the following hosting providers: ${[...SUPPORTED_HOSTING_PROVIDERS].join(', ')}`);
        }
        if (jsonRecap) {
            await writeFileAsync(path.join(outdir, `eleventy-plugin-content-security-policy-config.json`), JSON.stringify(config, null, 2));
        }
    });
};
